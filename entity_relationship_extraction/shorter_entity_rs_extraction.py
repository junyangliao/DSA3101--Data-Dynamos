...
def process_chunk(args):
    ...
    if col in ['description']:
        chunk['skill_entities'] = chunk[col].apply(
            lambda text: [(skill, 'SKILL') for skill in extract_skills_using_phrasematcher(text, matcher)])
...
def extract_entities_rs(csv_file_path): 
    ...
    for col in target_cols:
        if col in df.columns:
            ...
            chunk_data = [(chunk, matcher, col, new_entity_col, entity_type) for chunk in chunks]
