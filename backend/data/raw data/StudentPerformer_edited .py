...
def assign_majors(faculty):
    ...
    if faculty not in ["Yong Loo Lin Sch of Medicine", "Dentistry", "Law"]:
        ...
        if random.random() < 0.2:
            ...
            eligible_faculties = [
                fac
                for fac in faculty_to_majors
                if fac != faculty
                and fac not in ["Dentistry", "Law", "Yong Loo Lin Sch of Medicine"]
            ]
...
data["UE"] = data["attributes"].apply(
    lambda x: 1 if isinstance(x, str) and "su" in x else 0
)
