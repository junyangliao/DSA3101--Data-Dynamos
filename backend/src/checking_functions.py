...
def ontology_consistency_check(driver, ontology):
    with driver.session() as session:
        ...
        required_relationships = {rel: details for rel, details in ontology["relationships"].items() if not details.get("optional", False)}
        for relationship, details in required_relationships.items():
            source, target = details["from"], details["to"]
            relationship_issues = session.execute_read(check_required_relationships, source, {relationship: details})
            issues.extend(relationship_issues)

        # 3. Check orphaned relationships
        for relationship, details in ontology["relationships"].items():
            source, target = details["from"], details["to"]
            orphan_issues = session.execute_read(check_orphaned_relationships, relationship, source, target)
            if orphan_issues:
        ...
        for entity, data in ontology["entities"].items():
            ...
            attribute_issues = session.execute_read(check_attributes, entity, attributes)
            issues.extend(attribute_issues)
        
        # Return all issues found or a success message
        return {"status": "inconsistencies_found", "issues": issues} if issues else {"status": "consistent", "message": "No inconsistencies found"}
