...
def check_attributes(tx, entity, attributes, ontology):
    ...
    for rel_name, rel_details in ontology["relationships"].items():
        if rel_details.get("optional", False) and rel_details["from"] == entity:
            attr_name = (
                rel_name.replace("_IN", "").title().replace("_", " ").replace(" ", "_")
            )
...
def filter_ontology(ontology, existing_entities, existing_relationships):
    ...
    return {
        "entities": {
            k: v for k, v in ontology["entities"].items() if k in existing_entities
        },
        "relationships": {
            k: v
            for k, v in ontology["relationships"].items()
            if k in existing_relationships
            and v["from"] in existing_entities
            and v["to"] in existing_entities
        },
    }
    ...
    if results["status"] == "inconsistencies_found":
        ...
        for issue in results["issues"]:
            for check_type, details in issue.items():
                ...
                issue_type = (
                    check_type.replace(f"{entity}_", "").replace("_", " ").title()
                )
...
def ontology_consistency_check(driver, ontology):
    ...
    filtered_ontology = filter_ontology(
        ontology, existing_entities, existing_relationships
    )
    ...
    with driver.session() as session:
        ...
        for entity, data in filtered_ontology["entities"].items():
            ...
            attribute_issues = session.execute_read(
                check_attributes, entity, attributes, ontology
            )
