        ...
        for entity in row[entity_column]:
            ...
            if entity_type in representative_entities:
                if entity_type == "Module" and "moduleCode" in row.index:
                    ...
                    if not unique_value:
                        raise ValueError(
                            f"Missing unique identifier {unique_key} for {entity_type}."
                        )

                    attributes = {
                        k: v for k, v in row.items() if k in config["attributes"]
                    }
                    query = f"""
...
def create_relationship(
    tx, from_type, from_id, to_type, to_id, relationship_type, ontology
):
    relationship_config = ontology["relationships"].get(relationship_type)
    if (
        not relationship_config
        or relationship_config["from"] != from_type
        or relationship_config["to"] != to_type
    ):
        raise ValueError(
            f"Invalid relationship {relationship_type} between {from_type} and {to_type}."
        )
...
def module_helper(tx, row):
    ...
    semesters = []
    if row["semester_01"] > 0:
        semesters.append(1)
    if row["semester_02"] > 0:
        semesters.append(2)
    if row["semester_03"] > 0:
        semesters.append(3)
    if row["semester_04"] > 0:
        semesters.append(4)
...
def generate_cypher_query(tx, prompt):
    ...
    json_file_path = os.path.join(
        os.path.dirname(__file__), "ontology_config_test.json"
    )
